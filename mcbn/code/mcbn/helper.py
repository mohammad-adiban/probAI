...
def __return_yaml(file_name):
    wd = os.getcwd()
    setup_path = os.path.join(wd+'/mcbn/code/mcbn', file_name)
...
def get_lambdas_range(flt_min, flt_max):
    ...
    return np.logspace(int(exp_max), int(exp_min), int(n_lambdas)).tolist()
...
def get_logger():
    ...
    fhandler = logging.FileHandler(filename='mcbn/code/mcbn/evaluation.log', mode='a')
