...
class Dataset(object):
    def __init__(self, split=0, prop=0.9):
        ...
        indices = np.arange(l)
        trainval_idx = indices[0:round(l*0.9)]
        test_idx = indices[round(l*0.9):-1]
        ###################
...
class Kin8mn(Dataset):
    ...
    url = uci_base_url + 'kin8nm/kin8nm.csv'
    def read_data(self):
        data = pandas.read_csv(parentdir + '/bayesian_benchmarks/data/kin8nm/kin8nm.csv', delimiter=',').values
        return data[:, :-1], data[:, -1].reshape(-1, 1)
...
class Power(Dataset):
    ...
    url = uci_base_url + 'power/power.csv'
    def read_data(self):
        data = pandas.read_csv(parentdir + '/bayesian_benchmarks/data/power/power.csv', delimiter=',').values
        return data[:, :-1], data[:, -1].reshape(-1, 1)
...
class Protein(Dataset):
    ...
    def read_data(self):
        data = pandas.read_csv(parentdir+"/bayesian_benchmarks/data/protein/CASP.csv", delimiter=',').values
...
class WineRed(Dataset):
    ...
    def read_data(self):
        data = pandas.read_csv(parentdir+"/bayesian_benchmarks/data/winered/winequality-red.csv", delimiter=' ').values
        return data[:, :-1], data[:, -1].reshape(-1, 1)
